...
namespace Brushblade.Core.Tests
{
    ...
    public class MetaTests
    {
        ...
        // ---- 防御轴的两条角色属性曲线(E-b4 T4,2026-08-12;2026-09-16 随护甲百分比化抬升)----

        // 2026-09-18 合并:封顶级 26 → 50 与护甲上限 12 → 30 汇总,每 5 级 +3、Lv50 触顶(用户拍板)
        [TestCase(1, 0)]    // 起点 0:护甲是土系字给的,不是白送的 —— 1 级行为与引入 DEF 之前逐字节相同
        [TestCase(4, 0)]
        [TestCase(5, 3)]    // ⌊5/5⌋×3 = 3,首次 +3
        [TestCase(11, 6)]
        [TestCase(26, 15)]
        [TestCase(49, 27)]
        [TestCase(50, 30)]  // ⌊50/5⌋×3 = 30,恰好触顶
        public void DefenseFor_GrowsThreePerFiveLevels_CapsAt30(int level, int expected)
        {
        ...
        public void DefenseAndDodge_StayCapped_FarAboveCapLevel()
        {
            ...
            // 80 级的自然值是 DEF 48 / 闪避 40,与封顶值差得远,删掉封顶这条必红。
            // (2026-09-16 封顶级推到 50 后,原来的 40 级已在封顶之前,换成 80。)
            Assert.That(MetaRules.DefenseFor(80), Is.EqualTo(30));
            Assert.That(MetaRules.DodgeFor(80), Is.EqualTo(25));
            Assert.That(MetaRules.DefenseFor(200), Is.EqualTo(30));
        ...
        public void AllFiveCurves_CapTogetherAtLevel50()
        {
            ...
            Assert.That(MetaRules.MaxHpFor(50), Is.EqualTo(1480));
            Assert.That(MetaRules.AttackFor(50), Is.EqualTo(198));
            Assert.That(MetaRules.DefenseFor(50), Is.EqualTo(30));
            Assert.That(MetaRules.DodgeFor(50), Is.EqualTo(25));
            Assert.That(MetaRules.SpeedFor(50), Is.EqualTo(125));
            Assert.That(MetaRules.MaxHpFor(49), Is.LessThan(1480));
            Assert.That(MetaRules.AttackFor(49), Is.LessThan(198));
            Assert.That(MetaRules.DefenseFor(49), Is.LessThan(30));
