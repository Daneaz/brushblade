...
namespace Brushblade.Core.Tests
{
    ...
    public class CharTableTests
    {
        ...
        ///
        /// ⚠⚠ **2026-09-16:这条补偿规则的依据已经作废,而紫档锚点 200 与 剁 的 110×2 均未重定。**
        /// 原依据是「点数护甲对多段有天然惩罚 —— 每段各扣一次 DEF」,配套算例是:
        /// 面对 DEF 30 的敌人,剁 打出 (110−30)×2 = 160,同档单段 220 伤打出 190,补偿前只打出 140。
        /// 护甲改百分比减伤(DR = 甲/(甲+100))后减伤是**乘区,对多段满足分配律** ——
        /// 每段各折与整发折一次逐位相同(守卫测试 DefenseWiringTests.MultiHit_AppliesDefensePerSegment
        /// 的判别表记着这笔账),**多段的天然惩罚不存在了**,补偿也就失去了它要补的东西。
        ///
        /// ⚠ **Task 11 重标定字表锚点时,别照着上面这条已作废的依据算。**
        /// 要不要保留 ×(1 + 0.1×(段数−1)) 这条补偿、紫档 200 该不该重定,是独立议题,
        /// 不在 2026-09-16 那一批的范围内 —— 本批只改注释,数值一个字没动。
        ///
        /// ⚠ **2026-09-16 Task 11 复核结论(用户拍板)**:上面这条依据确认作废,但紫档单段
        /// 锚点 200(以及 剁 的 110×2)**本批不重定** —— 数值评估后决定留着,要不要重定
        /// 是独立专项,不在本批改动范围内。
        ///
        /// 以下为失效的原始论证,留作对账:
