...
namespace Brushblade.Data
{
    ...
    public static class Strings
    {
        ...
        ///
        /// 漏传的占位符原样留在文本里(不抛)——同样交给对账测试拦。
        ///
        /// 同名占位符在模板里可以出现多次,但实参**只需传一次**:比如
        /// `"本场穿透+{value}(本场每次攻击无视 {value} 点护甲)"` 调用时只传一个
        /// `("value", 15)`。这是因为下面的 `Replace` 是全量替换,一次调用就把
        /// 该名字的所有出现都换掉 —— 别把「只传一次」当成调用点漏传,补第二次传参
        /// 反而会被 `Replace` 的全量替换语义覆盖,没有实际效果。</summary>
