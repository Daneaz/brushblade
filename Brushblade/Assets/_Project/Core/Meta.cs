...
namespace Brushblade.Core
{
    ...
    public static class MetaRules
    {
        ...
        public static readonly IReadOnlyList<string> StartingCollection = new[]
        {
            ...
            "枪", "藤", // 木:白 召唤·贯穿 / 绿 召唤·命中冻结
            "冷", "冻", // 水:白 减速1+49伤(支援面回血 33)/ 绿 减速2+63伤(回血 42)
            "灭", "热", // 火:白 封禁+全体 19 伤 / 绿 90 伤+灼烧 1
            // ⚠ 土系配的是蓝档 碎 而不是绿档 垒,与其余三系不同 —— 刻意:土系白档 碉 攻 0
            // (纯反伤肉盾),只配 垒(盾 30+护甲 4+60 伤)整系仍然偏软,配 碎 才让土系开局有输出。
            "碉", "碎", // 土:白 召唤·反伤 50% / 蓝 盾 46+91 伤+破甲 20
        };
        ...
        /// <summary>防御成长:⌊(等级−2)/4⌋,上限 12(19.2.1 角色属性,2026-08-12 E-b4 T4)。
        /// 2026-09-16:原为 ⌊(等级−1)/2⌋、26 级触顶;封顶级推到 50 时**上限不抬**(理由见下),
        /// 改为每 4 级 +1,首次 6 级,50 级恰好 12。<c>Math.Max(0, …)</c> 防 1 级出负数除法。**起点 0**:护甲是土系字给的,不是白送的 —— 也正因为起点 0,
        ...
        public static int DefenseFor(int level) => Math.Min(12, Math.Max(0, level - 2) / 4);
        ...
        /// 一次随机都不摇);斜率是 DEF 的 2 倍,因为闪避是概率轴 —— 满级 25% 的期望减伤
