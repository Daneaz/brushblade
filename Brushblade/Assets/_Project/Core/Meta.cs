...
namespace Brushblade.Core
{
    ...
    public static class MetaRules
    {
        ...
        /// 一次随机都不摇)。
        ///
        /// ⚠⚠ **2026-09-16:下面这条「k = 1 而非 DEF 的 1/2,…与 DEF 12 对 R_in=60 的
        /// −20% 同量级」的对齐论证已经作废,而数值本身未重新评估。** DefenseFor 已把斜率从
        /// 1/2 改到 3/2、封顶从 12 抬到 30(见该函数注释),这里依赖的 DEF 12 / k=1/2 两个
        /// 数都不再是 DefenseFor 的实际取值。闪避的 k 与封顶该不该跟着重新对齐,是独立议题,
        /// **不在 2026-09-16 那一批范围内**,这里原样保留 k=1、封顶 25。
        /// 原论证:k = 1 而非 DEF 的 1/2,因为闪避是概率轴 —— 满级 25% 的期望减伤
