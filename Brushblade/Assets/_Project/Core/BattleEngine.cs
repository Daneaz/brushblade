...
namespace Brushblade.Core
{
    ...
    public sealed class BattleEngine
    {
        ...
        /// <summary>攒一层厚/泉需要的量 = **固定 100**(2026-09-11 用户拍板)。
        ///
        /// 沿革:/10(2026-09-02) → /5(2026-09-05) → /7(2026-09-11) → **固定 100**。
        /// 前三次都挂在 <c>PlayerMaxHp</c> 上,而**那是个结构性错配**:被这个阈值 gate 的
        /// 盾/治疗值走的是 <see cref="ScaleByBaseAttack"/>(值 × PlayerAttack / 100),
        /// 两条成长曲线斜率差一倍 —— <c>MaxHpFor</c> 是 500 + 20×(L−1)(26 级 ×2.0),
        /// <c>AttackFor</c> 是 100 + 2×(L−1)(26 级 ×1.5)。**阈值涨得比它要卡的东西快一倍**,
        /// 所以每次锚点一动就得重标一次除数,三次都是同一个错配的不同表现。
        /// 更糟的是养元技能 <c>vigor</c> 累计 +600 MaxHp **只加血不加攻**,
        /// 满技能时阈值会被顶到 1600/7 = 228,而 圭 的盾仍是 124 —— 越养成越攒不动。
        ///
        /// 固定值一次性斩断这条耦合:阈值不再随等级/血量/技能漂移,玩家也读得懂。
        /// 旧注释担心的「固定 100 在深层一次给 6 层」是照旧锚点(㙓 630 盾)说的,
        /// 而「档位差距与升级替代」那批把锚点整列重定标后 㙓 群盾只有 138,
        /// **那个顾虑的前提已经不存在了**。
        ///
        ...
        /// 所以金档 圭(盾 83)单发不足一层不等于浪费 —— 三发 249 = 2 层 + 余 49。</summary>
