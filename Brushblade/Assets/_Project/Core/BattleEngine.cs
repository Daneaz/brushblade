...
namespace Brushblade.Core
{
    ...
    public sealed class BattleEngine
    {
        ...
        /// <summary>攒一层厚/泉需要的量 = 玩家生命上限的**七分之一**(2026-09-11,原为五分之一)。
        ///
        ...
        /// 「攒」这个过程根本不存在。
        ///
        /// ⚠ 2026-09-11 由 /5 改 /7:**这个阈值是照旧锚点标定的**,而「档位差距与升级替代」
        /// 那批把金档锚点整列 ×0.695(400 → 278),金系三张双方向字的防御面随之跌到
        /// 阈值以下 —— 圭 119→83、冰 146→99、杜 115→80,对阈值 100 全部变成「单发 0 层」。
        /// 用户裁定「3 次圭该叠 3 层厚」,即金档单发就该有 1 层。
        /// 取 /7 而不是 /6:金档最低的是 杜 80,/6 得 83 仍够不着,/7 得 71 三张都过线且有余量。
        /// 5/7 = 0.714 ≈ 金档锚点的 0.695 —— **阈值是按锚点同幅度回调的**,不是拍的。
        ///
        /// ⚠ 攒层本来就是**跨施放累计**的(见 GainStacks:accum 是字段不是局部量,
        /// 余数不丢、只有满层才清)。本次改的是「单发够不够一层」的门槛,不是累计与否。
        /// 下钳 1:MaxHp &lt; 7 时整数除会得 0,那会让 while 循环永不终止。</summary>
        private int ResourceThreshold => Math.Max(1, _config.PlayerMaxHp / 7);
