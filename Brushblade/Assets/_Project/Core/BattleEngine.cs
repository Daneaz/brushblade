...
namespace Brushblade.Core
{
    ...
    public sealed class BattleEngine
    {
        ...
        /// 不许各写一份:那正是两处口径分叉的起点。
        ///
        /// ⚠ **下限 1 是刻意的(2026-09-16 裁定)**,不是防越界的随手一钳。整数除法会在
        /// `damage × 100 &lt; 100 + 甲` 时截断到 0 —— 极端甲值下百分比减伤就**退化回了实质免疫**,
        /// 而那正是本批要消灭的东西。下限把「伤害永远进得去」这条卖点兑现到实现里。
        ///
        /// ⚠ 下限只在 `damage &gt; 0` 时生效,**不能写成无条件的 Math.Max(1, …)** ——
        /// 那会让本来就是 0 的伤害凭空变成 1 点(治疗折算、0 伤效果、空挥),是另一类静默 bug。
        ///
        /// 护甲在这里再钳一次 0 是防御性的:三个聚合函数已经钳过,但它们不是唯一调用面。</summary>
        internal static int ApplyDefense(int damage, int armor)
        {
            if (damage <= 0) return 0;
            return Math.Max(1, damage * 100 / (100 + Math.Max(0, armor)));
        }
        ...
        /// <c>AttackFor</c> 是 100 + 2×(L−1)(50 级封顶 ×1.98)。**阈值涨得比它要卡的东西快一倍**,
        /// 所以每次锚点一动就得重标一次除数,三次都是同一个错配的不同表现。
        /// 更糟的是养元技能 <c>vigor</c> 累计 +600 MaxHp **只加血不加攻**,
        /// 满技能时阈值会被顶到 1600/7 = 228,而 圭 的盾仍是 124 —— 越养成越攒不动。
        /// 100 → 200 不是重标定同一个错配,是配套 Task 11 的
        /// 联动调整:土/水系护盾量与治疗量将按 1:1 与攻击面对齐(整体约翻倍),
        /// 而攒厚的来源是**获得护盾量**、攒泉的来源是**治疗名义值**——阈值不同步翻倍的话,
        /// 土/水系上来就顶着 10 层满层(厚 +50% 伤害 / 泉 +50% 治疗),攒速直接超模。
        /// 阈值同步翻倍是为了在盾/治疗翻倍之后,保持与翻倍前同样的攒层节奏。
        ///
        /// ⚠ 本次改动与 Task 11(盾/治疗值本身翻倍)分两次落地:阈值先动,盾/治疗值后动。
        /// 中间那一段(仅此提交 ~ Task 11 落地前)是**明知的过渡态**——阈值已经翻倍、
        /// 盾/治疗量还没跟上,攒层会比两次改动都落地前慢一半,过这段就恢复原节奏。
        ///
        /// ⚠ 单发够不够一层与「攒不攒」是两件事。**攒层是三段全通的累计**,
        /// 一点都不丢:战斗内(<see cref="GainStacks"/> 的 accum 是字段不是局部量)、
        /// 跨战斗(<c>RunEngine._carriedShieldAccum</c>,战斗结束存、下一场传入)、
        /// 跨存档(<c>RunSnapshot.CarriedShieldAccum</c>,断点续爬不丢)。
        /// 所以单次护盾/治疗低于阈值不等于浪费 —— 累计跨过阈值一样计入一层
        ...
        /// 护甲挡的是「一次挥击」,所以出牌那两记、以及召唤物出手都照常吃敌人的护甲
            ...
            DamageSource source = DamageSource.None)
        {
            ...
            // 四个乘数全部删除,守方侧从此只剩下面那一句护甲折算(spec §4.1)。
            // ⚠ 2026-09-16 起那一句是**百分比减伤**(DR = 甲/(甲+100),见 ApplyDefense),
            // 不再是点数减法 —— 下面这两段关于「减法对乘法透明」的论证随之作废,留作历史对账。
            //
            ...
            // 乘法描述「这一击有多重」,护甲描述「这层皮有多厚」。
            // 2026-09-16 改百分比减伤后,减伤自己也成了乘区,实数下与生克/暴击可交换 ——
            // 但落点**仍然压在最后一步**:整数除法每挪一次位置就多截断一次,且
            // 「这一记实际落到身上多少」只该有一个读数。
            // 守卫测试 Order_CritBeforeDefense。⚠ 这条搬错**今天不会有任何测试变红**
            // (T2 全场护甲为 0),所以只能靠那条显式构造非 0 护甲的测试兜。
            //
            // 2026-09-16:百分比减伤天然到不了 0(除非原伤害本就 < 1 个甲的粒度),
            // 裁定 10 那条「下钳 0 不下钳 1」的取舍随点数减法一起作废。多段与 AOE 天然
            // 每记各折算一次 —— 本方法就是「一记」的粒度,每段/每目标各调一次。
            // 相克即破甲(2026-08-13 用户裁定):攻方属性克守方时,守方护甲**整层失效**。
            //
            // ⚠ 这不是上面那条「减免遭克制失效」补丁的复活 —— 那条是乘法层的代偿,修的是被按比例
            // 抽走的克制收益;护甲自成一个乘区后克制收益仍然一点没少(守卫测试
            // Defense_DoesNotEatCounterBonus),所以本条是一条**额外奖励**的新规则,不是修复。
            // 是开关不是折算:护甲再厚,相克这一记也整层无视(2026-09-16 改百分比后仍然如此 ——
            // 换的是护甲怎么折算,不是谁吃不吃护甲)。
            //
        ...
        private bool DamageSummon(int enemyIndex, int summonIndex, int damage, Element attacker)
        {
            ...
            // 护甲(2026-08-28,铠 可以挂给召唤物了;2026-09-16 改百分比减伤,推翻 E-b4 的点数减法):
            ...
            // 怪的线性成长甩开(与「低档字被点数护甲吃光」同型 —— 那个类比出自点数减法时代,
            // 2026-09-16 百分比化后伤害不会被吃光了,但「固定值追不上线性成长」这一层仍然成立)。
            //
        ...
        private void CastBossSkill(int index, EnemyState enemy, BossSkill skill)
        {
            ...
            switch (skill)
            {
                case BossSkill.Deluge: // 淹没:玩家挨双倍,召唤物各挨一下(不翻倍,仍是分摊主力)
                    // 2026-08-12(E-b4 T3):不再套额外的乘法减伤 —— 玩家份的护甲在
                    // DamagePlayerDirect 里折(2026-09-16 起是百分比减伤),召唤物份**不折**
                    // (召唤物没有自己的基础护甲,也不借玩家的,spec §4.2)
                ...
                case BossSkill.Devour: // 吞噬:无视血量必杀最前一只(不回血);没得吞就普攻(设计明确不 ×2,唯一例外)
                {
                    ...
                    {
                        // 没得吞退化成普攻:与普攻同口径过护甲;秒杀分支本身无数值可折,不动
