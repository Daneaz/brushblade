...
namespace Brushblade.Core
{
    ...
    public sealed class CampaignConfig
    {
        ...
        /// 护甲形同虚设。于是取一半:<c>defScale = 1 + (scale − 1) / 2</c>。
        ///
        /// ⚠⚠ **2026-09-16:「为什么不是同速」这一半立论已经作废,而数值本身未重新评估。**
        /// 原论证是「点数减法对小数值是**开关**不是削减,同速会让深层的低伤字全部归零,
        /// 字库多样性被护甲单方面掐死」。护甲改百分比减伤(DR = 甲/(甲+100))后这个开关效应
        /// 不存在 —— 乘区只会把伤害按比例压薄,何况 <c>BattleEngine.ApplyDefense</c> 对正伤害
        /// 还有下限 1,**任何护甲值都归零不了**。半速该不该继续、该取几分之几,是一个独立议题,
        /// **不在 2026-09-16 那一批的范围内**,这里原样保留半速。
        ///
        /// ⚠ **2026-09-16 Task 11 复核结论(用户拍板)**:半速立论确认作废,但半速缩放本身
        /// **本批不动** —— 它牵涉 <see cref="ScaledDefense"/> 这条引擎逻辑,继续与否是独立议题,
        /// 需要专门排期评估,不在本批(纯数值脚本)范围内。
        ///
        /// 同理,判据(spec §6.3.2,守卫测试 LowestTierChar_StillDentsArmoredMobAtDepth20)里
        /// 「非零输出」那一半也已失去判别力:同速缩放不再会让它归零,改成同速不会让那条变红。
        /// 那条测试今天还守得住的只剩它写死的那个确切数值。
        ///
