...
namespace Brushblade.Core
{
    ...
    public static class EndlessGenerator
    {
        ...
        public static IReadOnlyList<EnemyDef> BuildFloor(EndlessConfig config, int depth, GameRandom random)
        {
            ...
            // 带甲每场最多 1 只(2026-08-29,spec §4.4(a) 的完成态)。
            // ⚠ 2026-09-16 护甲改百分比减伤(DR = 甲/(甲+100))后,**下面这条理由已经不成立**:
            // 减伤成了乘区,每个目标各按自己的比例缩,点数时代那个「打 N 个目标就损失 N × DEF」
            // 的 N 倍惩罚不存在了。本条闸子保留原样(要不要继续限制带甲怪成群是另一个议题),
            // 但它现在没有数值上的理由撑着 —— 谁要动它,先补一条新理由。
            // 以下为失效的原始论证,留作对账:点数护甲对 AOE 有 N 倍
