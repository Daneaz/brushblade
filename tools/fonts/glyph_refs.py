...
_MINIONS = [
    ...
    # 2026-08-29 补的五只(六系各补到 3 只)。灯花/墨溅/悬针/败笔 四只旧怪至今仍没有底稿,
    # 是另一笔账 —— 本次不顺手扩，免得把「补新怪」和「补旧账」混进同一个改动里
    Job("minion", "涂改", "涂", "tugai", note="心系奶妈,笔画像被反复涂抹改写,边缘毛糙层层压盖"),
    Job("minion", "铁画", "铁", "tiehua", note="金系反噬,笔锋硬如铁铸,转折处有崩刃的缺口"),
    Job("minion", "镇纸", "镇", "zhenzhi", note="土系肉墙,方正压秤,下盘沉得像块石条"),
    Job("minion", "洇痕", "洇", "yinhen", note="水系分裂,墨在纸上洇开,边界糊成两团"),
    Job("minion", "衍文", "衍", "yanwen", note="木系增生,笔画向外抽枝,越拖越长"),
    # 2026-09-03 补的十一只 —— 四只旧欠账(灯花/墨溅/悬针/败笔,实装已久却从没进过本表)
    # 加上 2026-09-02 补的七只护甲怪。至此 enemies.json 的 25 只杂兵**全部**有底稿。
    Job("minion", "灯花", "灯", "denghua", note="火系灼身,烛芯爆开的灯花,火星四溅"),
    Job("minion", "墨溅", "溅", "mojian", note="水系后排远程,一滴墨被弹开,墨点拖尾飞出"),
    Job("minion", "悬针", "针", "xuanzhen", note="金系后排锁人,悬针竖出锋如针垂悬,独眼"),
    Job("minion", "败笔", "败", "baibi", note="火系前排锁人,写坏的那一笔,笔锋炸开墨溃"),
    Job("minion", "枯笔", "枯", "kubi", note="木系低阶护甲,墨枯飞白最多,壳是干硬木质"),
    Job("minion", "火漆", "漆", "huoqi", note="火系低阶护甲,封缄火漆冷凝成壳,边缘钝圆"),
    Job("minion", "砚台", "砚", "yantai", note="土系低阶护甲,一方石砚,厚钝,攻最低"),
    Job("minion", "铜钤", "钤", "tongqian", note="金系低阶护甲,铜印方正,面有铸痕"),
    Job("minion", "版牍", "牍", "bandu", note="木系高阶护甲,简牍层层叠压,横向木纹"),
    Job("minion", "窑变", "窑", "yaobian", note="火系高阶护甲,窑火烧变形的陶,开片透火"),
    Job("minion", "宿墨", "宿", "sumo", note="水系高阶护甲,隔夜墨结壳,壳下渗稠墨"),
]
