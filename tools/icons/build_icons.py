...
ICONS = {
    ...
    # 近战:交叉双剑(2026-09-04)。此前近战按「默认不出 chip」省掉,于是战斗画面上
    # 一只近战单位的射程是零标记的 —— 「没有标记」既可能是近战、也可能是这一格没接
    # 射程显示,玩家分不出来。与 ranged 刻意不同形:那个是「一件远程兵器」,
    # 这个是「两把兵器交上了」,20px 下靠 X 的整体轮廓就能分辨,不靠细节。
    # 两条短护手在下半,把 X 与「叉/取消」那个符号区分开。
    "melee": (
