...
SHIELD_F, HEAL_F, GROUP_F, SUM_ATK_F = 0.65, 1.00, 0.60, 0.70
...
PRICE = {
 ...
 '光环攻':0.35,'光环盾':0.35,'叠战意':0.20,'随行治疗':0.30,'闪避':0.25,'远程':0.25,
 '入场护盾':0.25,
}
...
R=[]
def C(i,el,ra,form,traits,note=''): R.append(dict(id=i,el=el,ra=ra,form=form,traits=traits,note=note))
...
C('藤','木','绿','sum',['嘲讽'],'嘲讽梯队·低')
C('葬','木','蓝','aoe',['光环攻'],'非召唤木字→召唤协同')
C('箭','木','紫','atk',['贯穿','光环攻'],'同上')
C('楸','木','紫','sum',['远程','命中挂灼烧'])
C('荆','木','紫','sum',['荆棘','嘲讽'],'嘲讽梯队·中')
C('桤','木','紫','sum',['迅捷','随行治疗'])
C('林','木','金','sum',['迅捷','自愈'])
C('柘','木','金','sum',['嘲讽','荆棘'],'嘲讽梯队·高')
C('森','木','橙','sum',['荆棘','迅捷','自愈'])
C('桂','木','橙','sum',['光环盾','自愈'],'边缘辅助:放大全场召唤物')
C('藻','木','橙','sum',['光环攻','自愈'],'同上')
C('𣛧','木','红','sum',['光环攻','迅捷','荆棘'])
...
for c in R:
    ...
    if form=='atk': atk=bud(A['单攻'])
        ...
        sm=f"{A['召数']} 只 · {int(round(A['召血']*(1-ratio)))} 血 / {int(round(A['召攻']*SUM_ATK_F*(1-ratio)))} 攻"
    elif form=='dual_s':
...
def total(r):
    ...
    if isinstance(r['sm'],str) and r['sm']:
        import re as _r; m=_r.findall(r'(\d+)',r['sm']); v+=int(m[0])*int(m[2])
    return v
