...
R=[]
def C(i,el,ra,form,traits,note='',tank=0): R.append(dict(id=i,el=el,ra=ra,form=form,traits=traits,note=note,tank=tank))
...
for c in R:
    ...
    if form=='atk': atk=bud(A['单攻'])
        ...
        hp=int(round((A['召血']+A['召攻']*ATK_TO_HP*r)*(1-keep)))
        at=int(round(A['召攻']*(1-r)*SUM_ATK_F*(1-keep)))
        sm=f"{A['召数']} 只 · {hp} 血 / {at} 攻"
    elif form=='dual_s':
