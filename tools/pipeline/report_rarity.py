...
def selection_of(rec):
    """本期入选建议:已在字表的必留;元素变体是部件不是卡;其余要玩家认得且契合 ≥ 贴合。"""
    if rec["in_game"]:
        return "已在字表"
    if rec.get("group") == "元素":
        return "部件·非卡"  # 氵/灬/艹/刂… 进部件池,不做成卡
    known = rec["gb"] in (1, 2) or rec["char"] in H.NAME_COMMON
    return "建议入选" if known and rec["tier"] in ("核心", "贴合") else "暂不入"
...
全是字义不明的生僻字。红/橙两档一共 {count(everything, '红') + count(everything, '橙')} 个,
但里面只有 焚 是常用字,其余(燊/嶘/刲/漜)全生僻。**这意味着按当前入选标准,首发池没有红卡**。
要凑齐卡池金字塔(白 6~8 / 绿 8~12 / 蓝 8~12 / 紫 5~8 / 橙 2~4 / 红 1~2),两条路:
放生僻字进来,或者在入选池内部重新相对切档。这是你的判断,我没有替你改。
...
| ✅ 建议入选 | {sum(1 for r in everything if r['select'] == '建议入选')} 个 | 玩家认得(GB 一二级 / 人名高频)**且**契合 ≥ 贴合 |
| ⬜ 暂不入 | {sum(1 for r in everything if r['select'] == '暂不入')} 个 | 生僻、繁体,或字义与本系特性对不上 |
| 🔧 部件·非卡 | {sum(1 for r in everything if r['select'] == '部件·非卡')} 个 | 氵/灬/艹/刂 这类元素变体只进部件池,稀有度对它们没意义 |

合计建议入选 **{len(picked)}** 字。「终审」列留空——你填最终决定,`✅` 收 / `❌` 砍 / `?` 再议。
