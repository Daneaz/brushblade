...
`CheckBossPhase` 换阶时 `ChargeCounter = 0`、`IsCharging = false`——挨着现有的 `Burn = 0` 写,与"新字新体"完全同源,不引入新概念。
...
- 对玩家造 `Attack` 伤害(护盾可吸收,不被召唤物拦截)
- **同时**对每只存活召唤物各造 `Attack` 伤害,各自走五行(`ResolveEffect(Attack, [], boss.Element, summon.Element)`)
...
1. 对玩家造 `Attack` 伤害(护盾正常吸收,不被召唤物拦截)
