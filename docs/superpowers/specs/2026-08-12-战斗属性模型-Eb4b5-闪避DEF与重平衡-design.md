...
`剁` 今天 10×2 = 20 总。×10 后本应 100×2 = 200;按补偿规则 → **110 × 2 = 220 总**。

面对 DEF 30 的敌人:补偿后打出 `(110−30)×2 = 160`;同稀有度单段 220 伤打出 190;
补偿前 100×2 只打出 140。仍差 30 点,但多段在「两次过斩杀阈值」「两次触发受击后效」上有
独立收益,不追求完全拉平。

> ⚠ **2026-08-13 订正**:本节初稿正文误写 `120 × 2 = 240`,与上面那行公式矛盾
> (公式给 `200 × 1.1 = 220`)。**用户裁定以公式为准。**补偿存在是因为 N 段字比单段多吃
> `(N−1)` 次 DEF,系数必须正比于 `段数 − 1` —— 单段字代入才得 ×1.0(不需要补偿);
> 240 相当于 `1 + 0.1 × 段数`,那会让单段字也白拿 +10%。
...
#### 6.1.1 T8 的实测复核(2026-08-13)

复核方法:跑黄金轨迹工装(`tools/trace`,40 种子 × 4 画像)与六档仿真,从事件流里取
`EnemyAttack.Amount`(= R_in,含被盾吸收部分)与 `Damage.Amount`(= R_mob / R_boss,
按是不是 Boss 战分),按层段出中位数。**这三个数是实测,不是推导。**

| 参考量 | 设计取值 | 实测中位(按画像) | 结论 |
|---|---|---|---|
| `R_in` | **60** | 新手 56 / 小成长 63(词渊 83)/ 养成 64(词渊 76)/ 探针·对照 84 | ✅ **成立**。字林段 56~64 与 60 几乎重合;词渊段实测 76~84,设计值偏保守约 25%(推导时漏算了标点小妖 / 焦痕 / 缺笔妖带来的加攻)。**但换算出来的护甲点数按「实际减伤比例」验收是对的** —— 堆甲探针实测挨打中位 25,对照 84,减了 70%,与 §6.2 设计的「三张堆满 −65%」同量级。**不改。** |
| `R_boss` | **120** | 新手 105 / 小成长 130 / 养成 172 / 探针 247 | ✅ **成立**。中前期画像 105~172,120 落在中间。山 60 / 江 30 / 钧 30 三个点数不改。 |
| `R_mob` | **85** | 新手 300 / 小成长 373 / 养成 495 / 探针·对照 495 | ❌ **被实测证伪,实际是设计值的 3.5~5.8 倍。** 推导只数了字表的裸基础值,漏了生克(最高 ×4.5)、卡等级(卡 5 = ×1.4)、攻击力、`DoubleVsBurning`,以及「机器人/玩家永远出手里最强的那张」。 |

**`R_mob` 被证伪,但墨渍的护甲 20 仍然不改** —— 因为决定它的从来不是折算率,而是 §6.3.2 的
可执行判据 T3-V4(「深度 20 时最低伤害档的字打带甲小怪仍要有非零输出」)。按 T3-V4 反推,
墨渍的基础护甲上限是 **23**(24 就归零);从 20 抬到 23 对一记 400 伤的实际打击是 5% → 5.75%,
毫无意义。**折算率与判据在这里给出了相差 6 倍的答案,判据赢** —— 这正是 §6.3.2 已经写过的
「判据优先于折算率」,只是这次的倍数大得多。

**真正的结论是模型层面的,不是数值层面的**:字表的单击量级从 30 铺到 500+(含生克与卡等级),
而点数护甲是**一个减法常量**。同一件护甲对最低档字吃掉 65%、对红档只吃 6%(《技能机制详表》
1.4 节有完整表)。**一个点数值无法同时对两端有意义** —— 这不是配错了数,是点数制自带的性质。
要收窄它只有两条路(都超出 T8 的「只改 json」边界,留给下一个子项目):
① 压缩字表的伤害跨度;② 让敌人护甲按玩家实力而不是只按深度缩放。
...
### 10.6 T8 末尾读数与结论(2026-08-13)

**六档读数(300 种子)。T8 只改了一个数(剁 100×2 → 120×2),六档一格未动 ——
因为六档的出阵表全是火/水/土,`剁` 是金系,不在任何一档的池子里。这是「测不出」,不是「没影响」。**

| 画像 | 均卒层 | P50 | P90 | 最深 | 达词渊 | 带甲多怪战/次 |
|---|---|---|---|---|---|---|
| 新手(1 级) | 6.2 | 6 | 8 | 10 | 0% | 0.0 |
| 小成长(3 级) | 7.8 | 8 | 10 | 13 | 2% | 0.0 |
| 养成(10 级) | 9.6 | 10 | 11 | 15 | 21% | 0.1 |
| 探针·对照(火系,深启 11) | 13.9 | 14 | 15 | 17 | 100% | 1.3 |
| 探针·土系堆甲(深启 11) | **14.5** | **15** | 16 | 19 | 100% | 1.4 |
| 探针·AOE 专精(深启 11) | **12.4** | **12** | 13 | 14 | 100% | 0.8 |

探针方向:堆甲 15 **>** 对照 14 ✅;AOE 12 **<** 对照 14 ✅(但这一档「因为错误的原因通过」,
见探针表的警示,不得据此校准 AOE 轴)。

#### 本批对难度的净影响:**+0.3 / +0.1 / +0.4 层(略微变简单)**

拿 T1 开工前的 `863a174` 跑同一把尺子:

| 画像 | 本批开工前(863a174) | 本批完成后(T8) | 差 |
|---|---|---|---|
| 新手 | 5.9 | 6.2 | +0.3 |
| 小成长 | 7.7 | 7.8 | +0.1 |
| 养成 | 9.2 | 9.6 | +0.4 |

量级 ×10、点数护甲、破甲复原、闪避、`Campaign.Scale` 去 `ceil`、焦痕 +50%、
缺笔妖组合被加强 —— 全部加总的净效果是**略微变简单**(玩家新得的 DEF/闪避两条轴
盖过了敌人侧的加强)。**这批不欠难度的账。**

#### ⚠ 但难度确实离第 20 章 20.4 的校准目标很远,而且**不是本批造成的**

20.4 白纸黑字写着「校准后机器人卒层:新手 8.6 / 卡 3 级 P50=10 / 卡 5 级 P50=15」。
今天是 6.2 / 8 / 10。T8 用同一把尺子在历史提交上跑了三个探点,一次定位:

| 提交 | 日期 | 新手 | 小成长 | 养成 |
|---|---|---|---|---|
| `4c54543` 无尽卒层仿真 + Boss 滞后缩放校准 | 07-17 | **8.6** | **10.0** | **14.7** ← 20.4 的数就是这里来的 |
| `c58a402` **出字即消耗 —— 废止战后回归** | 07-19 | **5.0** | **5.6** | **6.1** ← **一个提交塌了 42% / 44% / 59%** |
| `a46ffda` 战利品双排 5 选 2 + 掉落 2→3 | 07-19 | 5.5 | 6.0 | 7.0 ← 对冲只补回一半 |
| (本批开工前) `863a174` | 08-12 | 5.9 | 7.7 | 9.2 |
| **T8 末尾** | 08-13 | 6.2 | 7.8 | 9.6 |

→ **20.4 的 8.6/10/15 是「出字即消耗」拍板之前的数,作为目标带已经过期。**
这是一次用户拍板的玩法转向(3.8.1 废止),不是 bug;它的对冲(掉字 2→3、战利品 5 选 2)
只补回约一半,此后一个月的迭代又慢慢爬回 6.2/7.8/9.6。
**要不要把这 3~5 层补回来,是一次独立的重平衡决策,不在 T8 的「只改 json」边界里
——它要么改 20.4 的目标带,要么动玩法侧的资源产出。**

#### 前面几步推给 T8 的五笔账,逐条处置

| # | 账 | 处置 | 依据 |
|---|---|---|---|
| 1 | `剁` 多段补偿 100×2 → **110×2** | ✅ **改了**(走详表 → 重跑管线) | §4.4(b) 的补偿规则。⚠ 初稿正文误写 240,与公式矛盾;2026-08-13 用户裁定**以公式为准** = `200 × 1.1 = 220` |
| 2 | `Campaign.Scale` 去 `ceil` 的数值后果 | ❌ **不吸收** | 去 `ceil` 让敌人 **HP 与攻击同时**掉 2~8%(实测低攻杂兵 20 均 −8.1%、最大 −26.7%),净效果是**变简单**,与 20.4 目标带同向;而敌人基础值必须保持 10 的倍数(`Scaled` 用 `Round` 的正确性依赖这条不变量),最小调整粒度是 10 = 对攻 20 的杂兵 +50%,**粒度比偏移大 6 倍,没法吸收** |
| 3 | `焦痕` `ScorchGain` 固定 +2 → +50% | ❌ **不改**,且**测不出** | ① `ScorchGain` 是 `BattleEngine` 的常量,T8 不许动代码;② 焦痕只在词渊(11 层)起的池子里,三档基线达词渊 0%/2%/21%,**基线画像基本遇不到它**;深层变强这条方向是对的(固定值在深层被稀释) |
| 4 | `缺笔妖 + 标点小妖` 组合被加强 +87% | ❌ **不改**,但**已量到** | 变异实测:缺笔妖攻 30 → 20(−33%)只让三档动 **+0.2 / +0.2 / +0.2 层**。纸面 +87% 的尖峰是真的,但要 4 回合才成熟,实战权重很小。json 侧唯一的杆是缺笔妖基础攻,**为 0.2 层砍掉它 33% 的身份不划算**;精确的杆(`PunctuationBuffPercent`、`BaseAttack *= 2` 与百分比增益的相乘)全在代码里 |
| 5 | 第 10 章 / 详表的锚点推导表 | ✅ **重做了** | 见《技能机制详表》1.4 节 |
