...
**属性行的例外(通假 / 生僻,2026-07-30 补丁 Finding 2)**:

`MinionDetail` 顶部的属性行默认打印 `CharInfo.ElementName(def.Element)`,但两种怪不能这么做:

- **通假字**:真身与伪装每局重摇(`EnemyState` 构造时,`EnemyDef.Element` 对它不作数——见 `EnemyDef.cs` 注释),显示 `def.Element` 既非真身也非伪装,是纯错误信息。
- **生僻字**:配置的 `Element` 就是真属性,直接显示 = 泄底,与战斗格显示的 `?`、详情 chip 的「生僻」标签自相矛盾。

两者的属性行改为不含任何具体五行名的短标记:通假 = `属性现摇`,生僻 = `属性隐藏`。详细机制仍由下方的【通假】/【生僻】卡片承担,属性行不重复展开。血、攻两个数值不受影响,照常显示。
...
**2026-07-30 补丁(Finding 4)**:早先定稿的坚壁文案是「以守为攻:该阶段不放大招」——「以守为攻」暗示这是主动的防御增益,但上表里坚壁恰有多个位次 `DamageTaken == 1.0`(冰天雪地「冰」、山崩海啸「山」、气吞山河「山」),那些阶段没有任何减伤机制支撑这句话。文案改为只陈述事实:「该阶段不放大招」,不再暗示存在减伤;真正的减伤与否仍由本节独立追加的 `DamageTakenText` 行反映(有减伤才会出现那一行)。`Theme.BossSkillChipColor` 的坚壁 chip 底色未改动——它按技能类别着色(坚壁 = 防御流派标签),不针对单个阶段的具体承伤值,不在本次修法范围内。

### 5.2 蓄力机制脚注

```
蓄力:Boss 打 1 回合、蓄力 1 回合(不出手),第 3 回合放当前预告的大招,如此循环。
...
2026-07-30 二次修正,Finding 1:先前的措辞「每 2 个敌方回合蓄力一次」会被读成「每 2 回合一轮」,
但 `BossChargeEvery` 是**计数阈值**不是周期——`BattleEngine.ResolveBossTurn` 的实际节拍是
「普攻 → 蓄力(不出手) → 释放」三个敌方回合一轮,按现文案玩家会把大招落点算早一回合。
现改写成逐回合的节拍描述,不再用「每 N 回合」这种容易被读成周期的措辞。）

第二句是玩家最需要知道的 —— 它推翻了「顶个树就没事」的直觉,而那正是这套 Boss 技能系统要解决的原始问题。

**文案里的「1」「1」「3」** 由私有常量 `EnemyInfo.ChargeEvery`(同步自 `BattleConfig.BossChargeEvery`,默认 2)推导:攻击回合数 = 阈值 − 1,释放落在第 阈值 + 1 回合。`EnemyInfo` 拿不到 `BattleConfig` 实例,故以常量写入并加注释指向该字段;改配置需同步文案(与 2.节的维护耦合结论一致)。
...
```
墨渍         承伤 ×0.7:伤害打折 —— 但用克制它的属性打,减免完全失效
错字鬼/夯土妖  无特殊机制 · 纯数值对拼
```

墨渍与 Boss 的坚壁走**同一条规则**(`DamageTaken` 遇克制失效),文案措辞刻意保持一致 —— 玩家学一次就能套用到所有减伤敌人。现有的「被克制可破」措辞过于含糊(「可破」没说清是打折还是失效),一并替换。

**2026-07-30 回写(Finding 5)**:上面这行最初定稿是「伤害打七折」(只为墨渍 0.7 写死)。但 `DamageTakenText` 由 `PhaseDetail`/`MinionDetail` 共用,承伤系数不止 0.7 一档(六只成语 Boss 里出现过 0.5、0.75),写死"七折"对其他数值就是错的。实现把措辞泛化成「伤害打折」,数值仍由 `承伤 ×{damageTaken}` 精确给出(如 `承伤 ×0.5`、`承伤 ×0.75`),没有损失精度。本节回写以与实现对齐,而不是把实现改回写死的「七折」。
