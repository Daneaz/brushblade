...
⚠ **列数必须与所在表的表头一致。** 管线的 `_parse_row` 靠 `len(cells) < 4` 与 `len(cells[0]) != 1`
判行,列错位不会报错、只会让抽出来的「效果配置」取错格。改之前先看那张表的表头有几列,
下面给的行已经按各表的真实列数写好了 —— **照抄,不要自己删格子**。

**火系「攻 · 单攻」表(6 列:字 | 稀 | 效果配置 | 相生 | 最终值 | 实现)**
...
**水系「治 · …」表(7 列:字 | 稀 | 二级 | 效果配置 | 相生 | 最终值 | 实现)**
...
**土系「防 · 减伤 / 免伤」表(6 列)—— 杜 / 岿 / 塞 三字都在这张表里**
...
⚠ **规格更正**:spec §四 把 `杜` 写成 Wood(从配方 木+土 推的),**那是错的**。详表把 杜 归在
**土系**章节里,而管线是按章节判属性的 —— 详表是唯一真相,以它为准。这不影响相生
(杜 属土,火生土,配方 木+土 无火 → 不吃 ×3),生成出来的 `element` 会是 `Earth`。
