...
namespace Brushblade.CoreTests
{
    ...
    public sealed class AttackStatTests
    {
        ...
        {
            var engine = Battle(BattleConfig.AttackBaseline, "甲");
            engine.DebugApplyPlayerAttackBuff(50);
...
Expected: 编译失败 —— `BattleConfig.AttackBaseline`、`BattleConfig.PlayerAttack`、`BattleEngine.EffectiveAttack`、`BattleEngine.DebugApplyPlayerAttackBuff` 都不存在。
...
```csharp
        ...
        ///
        /// 用 public 而不是 internal:Tests 是独立 asmdef,internal 看不见,
        /// 而项目里没有 InternalsVisibleTo 的先例 —— 为一个方法引入那套机制不划算。
        /// SourceId 每次唯一(同 HoT / 焦痕自燃的做法):Apply() 会按 SourceId 覆盖同源条目,
        /// 不给唯一 id 的话第二次挂增益会覆盖第一次而不是叠加。</summary>
        public void DebugApplyPlayerAttackBuff(int amount)
        {
...
```csharp
        ...
        public void Bleed_DoesNotRetroactivelyScale()
        {
            ...
            engine.Cast("己", 0);
            engine.DebugApplyPlayerAttackBuff(100);
            var bleed = engine.Enemies[0].Statuses.Find(StatusKind.Bleed);
...
**留给用户在 Unity 里做的**:
1. 为新建的 `AttackStatTests.cs` 生成 `.meta` 并提交
2. 跑一次 EditMode Test Runner
