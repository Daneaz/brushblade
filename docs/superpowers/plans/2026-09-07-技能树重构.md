...
- [ ] **Step 7b: 把 `PerkView` / `PerkInfo` 降到「临时可编译」**

⚠ **这一步不能省。** `PerkDef` / `PerkRules.All` / `PerkRules.PerkLevel` / `TryUpgradePerk` /
`PerkEffect.Shield` 全被删掉了,而 `PerkView.cs` 与 `PerkInfo.cs` 引用了它们 ——
不改的话 **Presentation 从 T1 一直断到 T9**,而 T2/T3/T5 的验收步骤都要求离线编译通过。

本步**不做新 UI**(那是 T9),只把两个文件改到能编过且能用:

- `PerkInfo.ShortEffect(PerkNodeDef def)` / `Detail(PerkNodeDef def)`:按 `def.Effect` 返回
  一句话。**新增的那些 PerkEffect 暂时全部落到一个通用分支**(如
  `Strings.T("perk.info.effect.generic", ("value", def.Value))`),不要为每种写文案 —— T9 才写。
- `PerkView`:把 2×2 牌阵换成**一列滚动列表**,每行一个节点(名字取 `def.Id`、副标题取
  `PerkInfo.ShortEffect`、按钮调 `PerkRules.TryUnlock`),按 `def.Tree` 分三段。
  锁着的节点置灰。够用即可,能点能扣钱能亮起。

字符串表加一条 `perk.info.effect.generic`(如 `"效果 +{value}"`)。⚠ 这条是玩家可见文案,
按 CLAUDE.md 要确认字体子集 —— 用 `python3 -m pytest tools/fonts/tests/ -q` 判断,
红了才跑 `python3 tools/fonts/subset_fonts.py` 再复跑。
...
```bash
git add Brushblade/Assets/_Project/Core/Perk.cs Brushblade/Assets/_Project/Core/Meta.cs Brushblade/Assets/_Project/Tests/ Brushblade/Assets/_Project/Presentation/ Brushblade/Assets/StreamingAssets/config/strings.zh-CN.json
...
本任务接手的是 T1 Step 7b 留下的**临时列表版** —— 现在把它做成设计稿上的样子。
