...
IDS = Path(__file__).parent.parent / "data" / "raw" / "ids.txt"
SPEC = ROOT / "docs/design/字选型/技能机制详表.md"
...
def test_stack_chars_use_manual_recipe_not_ids():
    ...
    chars = build_chars(IDS.read_text(encoding="utf-8"),
                        {"燚": {"element": "Fire", "rarity": "Gold", "effects": []}})
...
def test_non_stack_char_recipe_comes_from_ids():
    """非叠字走 IDS 一级拆解:烫 = 汤+火。"""
    chars = build_chars(IDS.read_text(encoding="utf-8"),
                        {"烫": {"element": "Fire", "rarity": "Purple", "effects": []}})
...
- Test: `Brushblade/Assets/_Project/Tests/ConfigLoaderTests.cs`
...
`ConfigLoaderTests.cs` 追加。本文件的既有风格是 `ConfigLoader.LoadGraph(json字符串)`,
但这里要验的是**实际配置文件**,所以读文件:

```csharp
        private static RecipeGraph RealGraph()
        {
            // 从仓库根往下找实际配置(测试工装 cwd 是 tools/coretests)
            var path = System.IO.Path.GetFullPath(System.IO.Path.Combine(
                TestContext.CurrentContext.TestDirectory,
                "../../../../../Brushblade/Assets/StreamingAssets/config/chars.json"));
            return ConfigLoader.LoadGraph(System.IO.File.ReadAllText(path));
        }
...
> 若 `TestContext.CurrentContext.TestDirectory` 的相对层数不对,先跑一次打印它,
> 再调整 `../` 的个数。
...
`ConfigLoaderTests.cs` 追加(`RealGraph()` 是 Task 6 加的 helper):
