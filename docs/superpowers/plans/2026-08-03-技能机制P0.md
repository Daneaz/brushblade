...
# (字, 效果, 期望基础值, 上一档锚点)
for cid, kind, base, ceiling in [('焚','DamageAll',7,24), ('燊','DamageAll',8,30),
...
```csharp
        ...
        public bool SlowActs { get; internal set; }  // 半速开关:下一次预处理时按此旧值判定
```
...
⚠️ **本步原先给的代码有 bug,已于执行中修正(2026-08-03)。** 原代码是
「先翻转 `SlowActs` 再判定」,推演可知第 1 个减速回合会**出手**而非跳过
(初值 false → 翻成 true → `!true` 为假 → 不 continue),与 Step 1 测试断言的
「第 1 回合跳过」直接矛盾。且它只处理主行动循环,漏了 `EnemyAbility.Buff` 的
辅助加攻循环 —— 减速跳过的标点小妖照样能给同伴加攻。

**正确做法:把翻转收敛为整个 `EndTurn()` 里唯一一次写入,下游只读。**
在敌方行动阶段**最前面**加一段预处理,算出本回合每个敌人是否跳过:

```csharp
            // 半速预处理(2026-08-03):翻转只在此一处发生,下游两个循环只读 slowSkip。
            // 冻结优先——冻结期间减速节拍暂停(SlowTurns 不消耗、SlowActs 不翻转)。
            var slowSkip = new bool[_enemies.Count];
            for (int i = 0; i < _enemies.Count; i++)
            {
                var enemy = _enemies[i];
                if (!enemy.Alive || enemy.FreezeTurns > 0 || enemy.SlowTurns <= 0) continue;
                enemy.SlowTurns -= 1;
                slowSkip[i] = !enemy.SlowActs;      // 先按旧值判定
                enemy.SlowActs = !enemy.SlowActs;   // 再翻转,留给下一回合
            }
```

然后在**辅助加攻循环**与**主行动循环**里各加一句只读判断:

```csharp
                if (slowSkip[i]) continue;
```

> 为什么不能在两个循环里各自翻转:`SlowActs` 是**翻转**语义(不像 `FreezeTurns` 是递减),
> 翻两次会互相抵消,半速节奏直接错乱。这个不变式由
> `Buffer_SlowActTurn_StillBuffsOthers` 锁住 —— 用非 Buff 敌人的主测试**测不出来**。
