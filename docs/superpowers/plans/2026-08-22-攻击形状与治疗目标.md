...
```csharp
            ...
            new CharDef("连", Element.Heart,
                effects: new[] { new EffectDef(EffectKind.DamageSingle, 40,
                    shape: TargetShape.Volley, shots: 3) }),
            // 劈斩:顺劈 + 斩杀 + 多段 + 穿透,用来钉「这三样只作用主目标」
            new CharDef("劈斩", Element.Heart,
                effects: new[] { new EffectDef(EffectKind.DamageSingle, 40,
                    shape: TargetShape.Cleave, shapePercent: 50,
                    hitCount: 2, pierce: 10, executeBelowPercent: 90, executeKills: true) }),
```
...
```csharp
        ...
        /// <summary>多段与穿透都只作用主目标:两侧各挨一下、照常吃满护甲。</summary>
        [Test]
        public void Cleave_HitCountAndPierceApplyToPrimaryOnly()
        {
            // 三只同排、各带 10 点护甲。主目标吃 hitCount 2 + pierce 10,两侧吃 1 下且不穿甲
            var armored = new EnemyDef("甲", Element.Heart, 200, 0, defense: 10);
            var engine = Engine(new[] { "劈" }, new[] { armored, armored, armored });
            engine.Cast("劈", targetIndex: 1);

            int primaryLoss = 200 - engine.Enemies[1].Hp;
            int sideLoss = 200 - engine.Enemies[0].Hp;
            Assert.That(sideLoss, Is.GreaterThan(0), "两侧确实挨了打(护甲没把 50% 的伤害吃干净)");
            Assert.That(primaryLoss, Is.GreaterThan(sideLoss * 2),
                "主目标多穿了 10 点甲,掉血超过两侧的两倍");
        }
        ...
        public void Volley_EveryShotIsFullDamage()
        {
            ...
            var engine = Engine(new[] { "连", "凿" }, new[] { Attacker(0) });
            engine.Cast("凿", targetIndex: 0);       // 先用 1 点伤害的字量出单发基准
            int oneHit = 200 - engine.Enemies[0].Hp; // 这是凿的伤害,只用来确认引擎活着
            Assert.That(oneHit, Is.GreaterThan(0));
