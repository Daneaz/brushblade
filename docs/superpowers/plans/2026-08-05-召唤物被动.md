...
| `Brushblade/Assets/_Project/Tests/CharTableTests.cs` | 真实配置接线守卫(防 `ConfigLoader` 漏接字段的静默失败) | 6 |
...
```csharp
        ...
        {
            var enemy = new EnemyState(new EnemyDef("靶", Element.Heart, 50, baseAttack));
        ...
        public void Curse_AppliesAfterAttackBuff()
        {
            ...
            var enemy = new EnemyState(new EnemyDef("靶", Element.Heart, 50, 4));
            enemy.Statuses.Apply(new StatusEffect
            {
...
⚠ `EnemyState` 的构造函数是 `internal EnemyState(EnemyDef def)`。若测试程序集调不到,改用引擎路径构造敌人(`Engine(...).Enemies[0]`)并对其 `Statuses` 施加状态 —— 断言逻辑不变。实现者按仓库现状二选一,不要为了测试放宽生产代码的可见性。
...
⚠ `ConfigLoaderTests.cs` 被 `tools/coretests` 工装排除(它引 `UnityEngine.Application`)。**这条测试因此只在 Unity EditMode 里跑得到** —— 实现者写完后在报告里注明「需用户在 Test Runner 跑一次」,不要因为工装没跑到就当它绿了。为了工装也有覆盖,Step 4 会在 `CharTableTests` 里补一条走真实 `chars.json` 的接线守卫。
...
追加到 `Brushblade/Assets/_Project/Tests/CharTableTests.cs`(与既有的 `RealConfig_PierceChars_CarryIgnoreArmorFlag` 放在一起):
...
⚠ **这两条在 Task 7 之前必然红**(`chars.json` 里还没有这 8 个字,`graph.Get` 会抛)。这是刻意的:它们钉死 Task 7 的验收标准。实现者本任务只需确认「因为字不存在而失败」而不是「因为字段没接上而失败」,并在报告里写明这两条留给 Task 7 转绿。
...
预期:除新加的两条 `RealConfig_*`(因字未落地而失败)外全绿。
...
预期:pytest 全绿;coretests 全绿 —— **Task 6 留下的 `RealConfig_SummonPassiveChars_CarryTheirPassive` 与 `RealConfig_GuiGrantsSummonShield` 此刻必须转绿**。
