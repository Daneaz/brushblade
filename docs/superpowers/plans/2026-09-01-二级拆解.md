...
到这一步语义才正式分叉。做完 `chars.json` 从 131 条变 141 条。
...
- Produces: `chars.json` 141 条(74 字 + 67 部件);12 个部件带 `recipe`;新增 10 个终点部件 `己 合 歹 品 丞 贝 几 七 开 覀`。
...
def test_real_table_entry_count():
    ...
    assert len(playable) == 74
    assert len(chars) == 141, "74 字 + 67 部件"
```
...
Expected: 输出 `141 条(字 74)`;pytest 全绿。
