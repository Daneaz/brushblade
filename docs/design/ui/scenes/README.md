...
两处栏宽有意偏离稿子（2026-08-28 反馈）：角色栏 214pt → 187pt、宝箱栏 228pt → 195pt，
让出的 125u 全给中间的书塔。两侧都是「看一眼就够」的信息，书塔才是每次进主界面要用的。
同一轮里续爬按钮上只留动作（「续 爬」），层段与层数搬到按钮下的详情行 ——
稿上「续爬 · 文山 44 层」在 523u 的胶囊里排不下。
