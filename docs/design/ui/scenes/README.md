...
| `Upgrade.dc.html` | 升级确认弹窗 | `CollectionView.ShowUpgradePreview` |
| `CardDetail.dc.html` | 字卡详情 · 重排(规范) | `CollectionView.BuildDetail` |
...
### 拖拽进出出阵表(2026-09-04 新增,稿上没有)

网格里的字**横着拽到右栏**松手 = 编入出阵;右栏的出阵格**横着拽出去**松手 = 卸下。
原来的「编入 / 卸下」按钮照旧,拖拽是第二条路,不是替代。

⚠ **竖向手势必须让给列表滚动**:收集网格坐在 `ScrollRect` 里,而 uGUI 的拖拽只发给
起拖的那个对象往上找到的第一个 `IDragHandler` —— 牌上一挂拖拽组件,竖着划就再也滚不动了。
`DragToDeck` 在起拖那一刻按方向分流:竖向把整条事件转发给祖先 `ScrollRect`,横向才算
「拿起这张牌」。判据用 `position − pressPosition`(按下到此刻的总位移),不是单帧 delta ——
单帧位移在阈值附近又小又抖,方向判不准。右栏的出阵格同理(它坐在右栏的滚动容器里)。

⚠ 拖拽期间**不许重绘**:组件一被销毁 `OnEndDrag` 就再也不来,字影会卡在屏幕上。
所以起拖时的落点高亮走的是「就地改右栏描边色 + 点亮一条预建好的提示带」,不是重建。

### 升级弹窗与字卡详情重排(2026-09-04,尚未接线)

**升级确认弹窗**原来是「Lv.1 → Lv.2」一行 + 前后两句效果全文对着看 —— 玩家得自己
在两串长句子里找哪个数变了。稿拆成两段:**数值提升**(一行一条「旧 → 新 (+差)」)、
**层数与特性**(灼烧几层、致盲几成,同一条读法,前面挂图标 chip);没变的收成底下一行灰字,
它回答的是「我会不会丢掉什么」。⚠ **不印重复卡与墨锭消耗**(用户拍板):
不够根本走不到这一页,详情页那颗按钮就是灰的,印出来只是把「你付得起」说第二遍。

**字卡详情**四段各回答一个问题 —— 多大 / 打谁 / 召什么 / 还带什么:

- **数值**格去掉下面那行小注(「单体,每次」之类),那句话交给「攻击模式」整段说;
- **攻击模式**一格一条(单体攻击 / 全体攻击 / 单体护盾 / 全体护盾 / 单体治疗 / 全体治疗),
  双方向字两面各占一格,前面的色点就是方向(朱砂=攻、翠玉=护);
- **召唤**单开一段,只有召唤字才出 —— 出去的不是一记伤害,是一只会自己行动的东西;
- 「字卡功能」改叫**特性 · 技能**,与 `UnitFoe` / `UnitAlly` 的 `.abil` 逐像素同款:
  图标 chip + 名 + 一行说明。图标走 `Icons.cs` 那 30 枚,没有图标的退成纯文字 chip
  (偷袭 / 嘲讽 / 斩杀 / 分段…),宽度与带图标的一致,行与行仍然对得齐。

**召唤字是另一条顺序**(2026-09-04 改稿):**召唤排在最前** —— 那几只是这张字的一切,
先看清召出来的是什么、几只,后面三段才有主语。召几只就画几只,**不写「×2」也不写说明**
(数出来的比读出来的快,血与攻就在下一段)。另外两段标题点名 **(召唤物)** ——
不点名的话玩家会把「攻击 44」读成这张牌自己的伤害。
召唤物的**攻击模式是近战 / 远程**(够不够得着后排),不是单体 / 全体。

**2026-09-04 当天已接线**:`CardTraits.cs` 是新写的那张「效果 → (图标, 名, 一句说明)」表 ——
`Modes()` 出攻击模式,`Of()` 出特性;32 个 `EffectKind` 与 `SummonPassive` 的每个字段各一条,
兜底分支把枚举名原样印出来(漏一条的表现是那条特性在详情里凭空消失,没有任何测试会红)。
`CollectionView.BuildDetail` 按四段重排,`ShowUpgradePreview` 换成两段式弹窗。

⚠ 那张表里的 key **必须逐个写成字面量**,不能拼(`"collection.trait." + slug + ".name"`):
StringsTableTests 的扫描只认写死的 key —— 拼出来的话全部文案会被判成孤儿、拼出的前缀被判成缺失。
2026-09-04 当场踩到一次,见 [[strings-table-scan-is-literal-only]] 那条老账。

### 出阵列表改用缩小版字卡(2026-09-04 新增,稿上没有)

卡组页右栏那 15 格、主界面角色栏的出阵预览,原先都是「属性色圆角格 + 字」——
只说得出**什么系**,说不出**什么档**,而出阵表里最该一眼看见的正是稀有度。
两处都改成 `Ui.MiniGlyphTile`:走同一套稀有度框素材,**不挂动效、不印拼音**。

整张 `Ui.GlyphTile` 塞进 50 宽的格子里,三样东西同时失效 —— 拼音小到看不清、
属性元件在 50px 上糊成一团、材质光效还在那儿一刻不停地跑(同屏 15 张)。
字形因此比大牌占得更满(0.46 牌高 vs 0.34):拼音那行让出来的位置全给它。

⚠ 牌一律按 **0.8 竖版比例**定尺(框素材是 192×240),格子的富余宽度让给间距,
不去拉牌 —— 拉了框上的纹样就变形。卡组格 74×92(下面再加一行等级,格高 117),
主界面 50×62 本来就是这个比例,排布一行没动。
...
## 2026-09-18 对比度调整（稿与 `Theme.cs` 同步）

设计系统「字·斗」（claude.ai artifact）按 WCAG AA 做了一轮对比度检查：正文 4.5:1，24px 以上或 19px 以上粗体 3:1。
下面七个色调深，**稿与 `Theme.cs` 一起改**。稿上是全局替换，所以同一个色值当描边、底色用的地方也跟着变深了：

| 色 | 原 → 新 | Theme | 为什么 |
| --- | --- | --- | --- |
| 图注灰 | `#88909C` → `#676F7A` | `LockGray` | 宣纸上 2.9 → 4.5 |
| 最弱标注 | `#B3AB9B` → `#928B7B` | （Theme 里没有） | 2.0 → 3.0，降级为装饰档：只给空格位、水印这类删掉也不丢信息的字 |
| 未拥有字形 | `#AFA695` → `#8E8675` | `LockedGlyph` | 2.0 → 3.0（大字） |
| 深金字 | `#8F6B09` → `#8A6600` | `GoldDeep` | GoldSoft 上 4.2 → 4.5。`GoldBorder` 仍是 `#8F6B09`，稿上不区分两者 |
| 商城赭 | `#A75A3D` → `#A05337` | `ShopNav` | 商城页签 4.1 → 4.5；页签底/描边按 `TabPalette.FromAccent` 重算成 `#F1E6DC` / `#D0AB9A` |
| 翠玉 | `#439458` → `#34864B` | `Jade` | 「可升」白字 3.7 → 4.5 |
| 土 | `#997C3C` → `#8F7332` | `ElementColor(Earth)` | 攻击模式 chip 白字 4.0 → 4.5（`UnitDetailChip.Positioning` 同源跟着变） |

两张稿另有按组件改的规则（**只改了稿，实现侧还没跟**）：

- `Home.dc.html`：层段刻度 `.bd .df` 由最弱标注改为图注灰（是必读信息）；页签副文 `.tab .td` 改 `#5D646F`（页签淡底上图注灰不到 4.5）。
- `Battle.dc.html`：敌人属性小标 `.foe .els` 底色 `--el` → `--glyph`（金系原色白字只有 2.5）；配字表「缺」、召唤物血量数字、字库/部件标签、可合成箭头、引导「?」各改深一档；血条上 6pt 白字的描边加重（它同时压在血条与轨道上，没有一种底色两边都达标）。

非激活态（未拥有、前置未点、等级未到、已阵亡、禁用）不在要求之内。面板描边 `#DED7C9` 对宣纸 1.3:1 保留（面板不是控件）。
⚠ `tools/design/mobs/svg` 里怪物光晕用的 `#997C3C` 是烤进立绘的美术色，**不跟**。

### 第二轮：把稿里剩下的不达标文字清干净（同日）

第一轮只同步了 `Theme.cs` 里那七个色，全稿自动核查还剩 369 处文字不达标。这一轮按类清理，**只动稿，不动 `Theme.cs`**：

- **暖灰并成一支 `#696151`**：稿上原有 `#7A7261`／`#A8A08E`／`#8A8478`／`#A09A8C` 等好几支暖灰小字，最低只有 2.2:1。
  只替换 `color:` 位置（同色值当描边、底色的地方不动）。层级改由字号与字重承担。
- **有色浅底上的小字升一档**：稀有度淡底上的拼音、属性淡底上的等级角标、`#E9E5DB` 之类灰底上的标签，
  由 `#676F7A`（图注）升到 `#5D646F`（说明），个别再升到 `#2C333D`。`.e-None` 的 `--elFg` 同理 `#5D646F` → `#2C333D`。
- **白字压浅色底**：属性圆点、五行环图节点、怪牌属性角标改压**字形色**；金底状态 chip 改压深金 `#8A6600`；
  结算的宝箱档位图标、技能页的金色按钮改成深字；开箱的绿色稀有度徽标压深到 `#09893F`。
- **飘字规范板**的展示底由 `#EFE9DA` 换成宣纸底 —— 飘字色是按宣纸底定的，规范板不该用另一种底去演示。
- **血条上的白色数值**（单位详情三张稿）跟战斗屏一样加重描边。
- **装饰档 `#928B7B` 只留给真装饰**：开箱步骤号、「空箱位」、图鉴「未遭遇」、技能页的基准值这些是必读信息，改用 `#696151`。

核查口径：逐个文字元素算它与实际背景（含半透明叠加）的对比度，正文 4.5:1、大字 3:1；
非激活态（未拥有／未解锁／已阵亡／禁用）、水印、安全区标注、带描边的血条数字不计入。
全部 35 张稿现在是 0 处不达标。⚠ 这套核查是逐元素量出来的，不是肉眼看的 —— 改稿后要重跑才算数。
